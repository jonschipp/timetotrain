...
class Workout:
  ...
  def generate_weeks(self, weeks: int, frequency: int) -> list:
      if not weeks:
          # Set default
          weeks = self.weeks
      ...
      for week in range(1, weeks + 1):
         ...
         self.generate_sheet_banner(
             row=1, col=BEGIN_COLUMN, currentSheet=currentSheet, value=f"Week {week}", frequency=frequency
         )
  ...
  def generate_sheet_banner(self, row: int, col: int, currentSheet: object, frequency: int, value: str = 'Item') -> None:
              # Get last column of spreadsheet for full banner
              end_column=frequency*(COLUMN_LENGTH+1)+frequency

              # Print week banner on first row
              currentSheet.merge_cells(
                  start_row=1, end_row=1,
                  # Calculate total columns in sheet
                  start_column=col, end_column=end_column
              )
              currentCell = currentSheet.cell(
                  row=1, column=col, value=f"{value}"
              )
              self.set_style(
                  currentSheet, currentCell, col,
                  fgColor=colors.WHITE, bgColor=COLOR_DARKRED,
                  size=28, width=20, font='Helvetica', bold=True
              )
              currentCell.alignment = ALIGNMENT
              ...
              currentCell = currentSheet.cell(
                  row=2, column=col, value=f"Generated by Time to Train on {date}"
              )
